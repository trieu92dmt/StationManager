...
{
    public class BC01ReportController : BaseController
    {
        // GET: SO100Report
        [ISDAuthorizationAttribute]
        public ActionResult Index()
        {
            var searchModel = (BC01ReportViewModel)TempData[CurrentUser.AccountId + "BC01ReportSearchData"];
            var tempalteIdString = TempData[CurrentUser.AccountId + "BC01ReportTemplateId"];
            var modeSearch = TempData[CurrentUser.AccountId + "BC01ReportModeSearch"];
            //mode search
            ...
            var pageId = GetPageId("/Reports/BC01Report");
            // search data
        ...
        public ActionResult ViewDetail(BC01ReportViewModel searchModel, Guid pivotTemplate, string modeSearch)
        {
            TempData[CurrentUser.AccountId + "BC01ReportSearchData"] = searchModel;
            TempData[CurrentUser.AccountId + "BC01ReportTemplateId"] = pivotTemplate;
            TempData[CurrentUser.AccountId + "BC01ReportModeSearch"] = modeSearch;
            return RedirectToAction("Index");
        }

        public ActionResult ChangeTemplate(Guid pivotTemplate, BC01ReportViewModel searchModel, string modeSearch)
        {
            TempData[CurrentUser.AccountId + "BC01ReportSearchData"] = searchModel;
            TempData[CurrentUser.AccountId + "BC01ReportTemplateId"] = pivotTemplate;
            TempData[CurrentUser.AccountId + "BC01ReportModeSearch"] = modeSearch;
        ...
        [ValidateInput(false)]
        public ActionResult BC01ReportPivotGridPartial(Guid? templateId = null, BC01ReportViewModel searchViewModel = null, string jsonReq = null)
        {
            #region commonDate
            ...
            if (searchViewModel != null && searchViewModel.FinishCommonDate != "Custom")
            {
                DateTime? fromDate;
                DateTime? toDate;
                _unitOfWork.CommonDateRepository.GetDateBy(searchViewModel.FinishCommonDate, out fromDate, out toDate);
                //Tìm kiếm kỳ hiện tại
                searchViewModel.FinishFromDate = fromDate;
                searchViewModel.FinishToDate = toDate;
            }
            #endregion
            var pageId = GetPageId("/Reports/BC01Report");
            var listSystemTemplate = _unitOfWork.PivotGridTemplateRepository.GetSystemTemplate(pageId);
            ...
            {
                ViewBag.Search = null;
                return PartialView("_BC01PivotGridPartial", null);
            }
            else
            {
                if (!string.IsNullOrEmpty(jsonReq))
                {
                    searchViewModel = JsonConvert.DeserializeObject<BC01ReportViewModel>(jsonReq);
                }
                searchViewModel.Plant = CurrentUser.CompanyCode;
                var model = _unitOfWork.BC01ReportRepository.GetData(searchViewModel);
                ViewBag.Search = searchViewModel;

                return PartialView("_BC01PivotGridPartial", model);
            }
        }

        [HttpPost]
        public ActionResult ExportPivot(BC01ReportViewModel searchViewModel, Guid? templateId)
        {
        ...
        public ActionResult GetConfigFieldEdit(int StockCode)
        {
        ...
        public ActionResult SaveEditReport(BC01ReportFormViewModel viewModel)
        {
            return ExecuteAPIWithoutAuthContainer(() =>
            {
                _unitOfWork.BC01ReportRepository.UpdateDataBC01(viewModel, CurrentAccountId: CurrentUser.AccountId);
        ...
        public ActionResult SaveCreateReport(BC01ReportFormViewModel viewModel)
        {
            return ExecuteAPIWithoutAuthContainer(() =>
            {
                _unitOfWork.BC01ReportRepository.CreateDataBC01(viewModel, CurrentAccountId: CurrentUser.AccountId);
                return _APISuccess(null);
            });
        }
        #endregion
