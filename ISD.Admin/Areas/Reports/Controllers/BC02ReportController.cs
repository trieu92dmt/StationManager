...
{
    public class BC02ReportController : BaseController
    {
        // GET: BC02Report
        [ISDAuthorizationAttribute]
        public ActionResult Index()
        {
            var searchModel = (BC02ReportViewModel)TempData[CurrentUser.AccountId + "BC02ReportSearchData"];
            var tempalteIdString = TempData[CurrentUser.AccountId + "BC02ReportTemplateId"];
            var modeSearch = TempData[CurrentUser.AccountId + "BC02ReportModeSearch"];
            //mode search
            ...
            var pageId = GetPageId("/Reports/BC02Report");
            // search data
            ...
            #region CommonDate
            var SelectedCommonDate = "ThisYear";
        ...
        [ValidateInput(false)]
        public ActionResult BC02ReportPartial(Guid? templateId = null, BC02ReportViewModel searchViewModel = null, string jsonReq = null)
        {
            #region commonDate
            ...
            {
                //Phân xưởng
                var workShopList = _context.PhysicsWorkShopModel.Where(p => p.Actived == true && (searchViewModel.WorkShop == null || p.PhysicsWorkShopCode == searchViewModel.WorkShop))
                                                                .OrderBy(p => p.OrderIndex)
                                                                .Select(p => new ISDSelectStringItem()
                                                                {
                                                                    id = p.PhysicsWorkShopCode,
                ...
                var customerList = (from bc in _context.BC01Model
                                    ...
                                    where (so.KUNNR != null && so.KUNNR != string.Empty)
                                    && (searchViewModel.CompletedFromDate == null || (bc.FinishDateChange.HasValue && searchViewModel.CompletedFromDate <= bc.FinishDateChange || !bc.FinishDateChange.HasValue && searchViewModel.CompletedFromDate <= bc.FinishDate))
                                    && (searchViewModel.CompletedToDate == null || (bc.FinishDateChange.HasValue && bc.FinishDateChange <= searchViewModel.CompletedToDate || !bc.FinishDateChange.HasValue && bc.FinishDate <= searchViewModel.CompletedToDate))
                                    ...
                                    && (searchViewModel.Customer == null || so.SORTL.Contains(searchViewModel.Customer))
                                    group so by new { so.KUNNR, so.SORTL  } into g
                                    orderby g.Key.KUNNR
                                    select new ISDSelectStringItem()
                                    {
                                        id = g.Key.KUNNR,
                                        name = g.Key.KUNNR + " | " + g.Key.SORTL,
                                    }).ToList();
            ...
            {
                ViewBag.Search = null;
                return PartialView("_BC02Partial", null);
            }
            else
            {
                if (!string.IsNullOrEmpty(jsonReq))
                {
                    searchViewModel = JsonConvert.DeserializeObject<BC02ReportViewModel>(jsonReq);
                }
                searchViewModel.Plant = CurrentUser.CompanyCode;
                var model = _unitOfWork.BC02ReportRepository.GetData(searchViewModel);
                ViewBag.Search = searchViewModel;

                return PartialView("_BC02Partial", model);
            }
        }

        public ActionResult ViewDetail(BC02ReportViewModel searchModel, Guid pivotTemplate, string modeSearch)
        {
            TempData[CurrentUser.AccountId + "BC02ReportSearchData"] = searchModel;
            TempData[CurrentUser.AccountId + "BC02ReportTemplateId"] = pivotTemplate;
            TempData[CurrentUser.AccountId + "BC02ReportModeSearch"] = modeSearch;
            return RedirectToAction("Index");
        }

        [HttpPost]
        public ActionResult ExportPivot(BC02ReportViewModel searchViewModel, Guid? templateId)
        {
