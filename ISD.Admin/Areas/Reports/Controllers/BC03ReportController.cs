...
{
    public class BC03ReportController : BaseController
    {
        // GET: BC03Report
        [ISDAuthorizationAttribute]
        public ActionResult Index()
        {
            var searchModel = (BC03ReportViewModel)TempData[CurrentUser.AccountId + "BC03ReportSearchData"];
            var tempalteIdString = TempData[CurrentUser.AccountId + "BC03ReportTemplateId"];
            var modeSearch = TempData[CurrentUser.AccountId + "BC03ReportModeSearch"];
            //mode search
            ...
            var pageId = GetPageId("/Reports/BC03Report");
            // search data
        ...
        [ValidateInput(false)]
        public ActionResult BC03ReportPartial(Guid? templateId = null, BC03ReportViewModel searchViewModel = null, string jsonReq = null)
        {
            #region commonDate
            ...
            {
                ViewBag.Search = null;
                return PartialView("_BC03Partial", null);
            }
            else
            {
                if (!string.IsNullOrEmpty(jsonReq))
                {
                    searchViewModel = JsonConvert.DeserializeObject<BC03ReportViewModel>(jsonReq);
                }
                searchViewModel.Plant = CurrentUser.CompanyCode;
                var model = _unitOfWork.BC03ReportRepository.GetData(searchViewModel);
                ViewBag.Search = searchViewModel;

                return PartialView("_BC03Partial", model);
            }
        }

        public ActionResult ViewDetail(BC03ReportViewModel searchModel, Guid pivotTemplate, string modeSearch)
        {
            TempData[CurrentUser.AccountId + "BC03ReportSearchData"] = searchModel;
            TempData[CurrentUser.AccountId + "BC03ReportTemplateId"] = pivotTemplate;
            TempData[CurrentUser.AccountId + "BC03ReportModeSearch"] = modeSearch;
            return RedirectToAction("Index");
        }

        [HttpPost]
        public ActionResult ExportPivot(BC03ReportViewModel searchViewModel, Guid? templateId)
        {
