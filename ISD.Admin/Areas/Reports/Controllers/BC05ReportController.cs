...
{
    public class BC05ReportController : BaseController
    {
        // GET: BC05Report
        [ISDAuthorizationAttribute]
        public ActionResult Index()
        {
            var searchModel = (BC05ReportViewModel)TempData[CurrentUser.AccountId + "BC05ReportSearchData"];
            var tempalteIdString = TempData[CurrentUser.AccountId + "BC05ReportTemplateId"];
            var modeSearch = TempData[CurrentUser.AccountId + "BC05ReportModeSearch"];
            //mode search
            ...
            var pageId = GetPageId("/Reports/BC05Report");
            // search data
        ...
        [ValidateInput(false)]
        public ActionResult BC05ReportPartial(Guid? templateId = null, BC05ReportViewModel searchViewModel = null, string jsonReq = null)
        {
            #region commonDate
            ...
            //Phân xưởng
            var allDepartmentList = _context.AllDepartmentModel.Where(p => p.Actived == true)
            ...
            ViewBag.AllDepartmentList = allDepartmentList;
            ...
            {
                ViewBag.Search = null;
                return PartialView("_BC05Partial", null);
            }
            else
            {
                if (!string.IsNullOrEmpty(jsonReq))
                {
                    searchViewModel = JsonConvert.DeserializeObject<BC05ReportViewModel>(jsonReq);
                }
                searchViewModel.Plant = CurrentUser.CompanyCode;
                var model = _unitOfWork.BC05ReportRepository.GetData(searchViewModel);
                ViewBag.Search = searchViewModel;

                return PartialView("_BC05Partial", model);
            }
        }

        public ActionResult ViewDetail(BC05ReportViewModel searchModel, Guid pivotTemplate, string modeSearch)
        {
            TempData[CurrentUser.AccountId + "BC05ReportSearchData"] = searchModel;
            TempData[CurrentUser.AccountId + "BC05ReportTemplateId"] = pivotTemplate;
            TempData[CurrentUser.AccountId + "BC05ReportModeSearch"] = modeSearch;
            return RedirectToAction("Index");
        }

        [HttpPost]
        public ActionResult ExportPivot(BC05ReportViewModel searchViewModel, Guid? templateId)
        {
