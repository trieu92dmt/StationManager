...
{
    public class BC07ReportController : BaseController
    {
        // GET: BC07Report
        [ISDAuthorizationAttribute]
        public ActionResult Index()
        {
            var searchModel = (BC07ReportViewModel)TempData[CurrentUser.AccountId + "BC07ReportSearchData"];
            var tempalteIdString = TempData[CurrentUser.AccountId + "BC07ReportTemplateId"];
            var modeSearch = TempData[CurrentUser.AccountId + "BC07ReportModeSearch"];
            //mode search
            ...
            var pageId = GetPageId("/Reports/BC07Report");
            // search data
        ...
        [ValidateInput(false)]
        public ActionResult BC07ReportPartial(Guid? templateId = null, BC07ReportViewModel searchViewModel = null, string jsonReq = null)
        {
            #region commonDate
            ...
            {
                ViewBag.Search = null;
                return PartialView("_BC07Partial", null);
            }
            else
            {
                if (!string.IsNullOrEmpty(jsonReq))
                {
                    searchViewModel = JsonConvert.DeserializeObject<BC07ReportViewModel>(jsonReq);
                }
                searchViewModel.Plant = CurrentUser.CompanyCode;
                var model = _unitOfWork.BC07ReportRepository.GetData(searchViewModel);
                ViewBag.Search = searchViewModel;

                return PartialView("_BC07Partial", model);
            }
        }

        public ActionResult ViewDetail(BC07ReportViewModel searchModel, Guid pivotTemplate, string modeSearch)
        {
            TempData[CurrentUser.AccountId + "BC07ReportSearchData"] = searchModel;
            TempData[CurrentUser.AccountId + "BC07ReportTemplateId"] = pivotTemplate;
            TempData[CurrentUser.AccountId + "BC07ReportModeSearch"] = modeSearch;
            return RedirectToAction("Index");
        }

        [HttpPost]
        public ActionResult ExportPivot(BC07ReportViewModel searchViewModel, Guid? templateId)
        {
