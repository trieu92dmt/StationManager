(function($) {
	...
	$.fn.dualselect = function(options) {
		...
		var createAvailableBox = function() {
			...
			var srcElement = jQuery('<select id="' + _dualselect_avl_id + '" name="' + _dualselect_avl_class + '" class="form-control ' + opt.dualselectClass + ' ' + opt.dualselectClass + '-avl ' + _dualselect_avl_class + '" multiple="multiple"></select>');
		...
		var createSelectedBox = function() {
			...
			var selElement = jQuery('<select id="' + _dualselect_sel_id + '" name="' + _dualselect_sel_class + '" class="form-control ' + opt.dualselectClass + ' ' + opt.dualselectClass + '-sel '+ ' ' + _dualselect_sel_class + '" multiple="multiple"></select>');
		...
		var doFilterAvl = function() {
			...
			if (avlTxt.trim().length > 0) {
				...
				options.each(function(idx, elm){
					var _elm = jQuery(elm);
					if (_elm.text().toLowerCase().indexOf(avlTxt.toLowerCase()) >= 0) {
						_elm.show();
					} else {
						_elm.hide();
					}
		...
		var doFilterSel = function () {
			...
			if (selTxt.trim().length > 0) {
				...
				options.each(function(idx, elm){
					var _elm = jQuery(elm);
					if (_elm.text().toLowerCase().indexOf(selTxt.toLowerCase()) >= 0) {
						_elm.show();
					} else {
						_elm.hide();
					}
		...
		var createFilterBoxes = function() {
			...
			var avlFilter = jQuery('<input type="text" id="' + _dualselect_avl_id + '_filter_text" name="' + _dualselect_avl_id + '_filter_text" class="form-control" placeholder="' + opt.filtersPlaceholder + '">');
			var selFilter = jQuery('<input type="text" id="' + _dualselect_sel_id + '_filter_text" name="' + _dualselect_sel_id + '_filter_text" class="form-control" placeholder="' + opt.filtersPlaceholder + '">');
			avlFilter.bind('keyup change', doFilterAvl);
			selFilter.bind('keyup change', doFilterSel);
			if (opt.avlTitle) {
				var avlTitle = jQuery('<h5><b>' + opt.avlTitle + '</b></h5>');
				avlFilterWrapper.append(avlTitle);
			}
			if (opt.selTitle) {
				selTitle = jQuery('<h5><b>' + opt.selTitle + '</b></h5>');
				selFilterWrapper.append(selTitle);
			}
