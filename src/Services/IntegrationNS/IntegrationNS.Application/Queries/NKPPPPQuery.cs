...
namespace IntegrationNS.Application.Queries
{
    public interface INKPPPPQuery
    {
        ...
        /// <returns></returns>
        Task<List<NKPPPPResponse>> GetNKPPPP(NKPPPPIntegrationCommand command);
    }

    public class NKPPPPQuery : INKPPPPQuery
    {
        ...
        public NKPPPPQuery(IRepository<ScrapFromProductionModel> nkppppRepo, IRepository<WorkOrderModel> woRepo, IRepository<DetailWorkOrderModel> detailWoRepo,
        ...
        public async Task<List<NKPPPPResponse>> GetNKPPPP(NKPPPPIntegrationCommand command)
        {
            #region Format Day
            ...
            //Get data
            var data = await query.OrderByDescending(x => x.WeightVote).ThenByDescending(x => x.CreateTime).Select(x => new NKPPPPResponse
            {
                //ID NKPPPP
