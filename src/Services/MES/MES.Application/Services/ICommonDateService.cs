...
{
    public interface ICommonDateService
    {
        bool GetDateBy(string CommonDate, out DateTime? FromDate, out DateTime? Todate);
        bool GetDateBy(string CommonDate, out DateTime? FromDate, out DateTime? Todate, out DateTime? FromPreviousDay, out DateTime? ToPreviousDay);
    }

    public class CommonDateService : ICommonDateService
    {
        private readonly IRepository<DimDateModel> _dimDateRep;

        public CommonDateService(IRepository<DimDateModel> dimDateRep)
        {
            _dimDateRep = dimDateRep;
        }
        ...
        public bool GetDateBy(string CommonDate, out DateTime? FromDate, out DateTime? Todate)
        {
            try
            {
                var dateKey = DateTimeExtensions.ConvertDateTimeToInt(DateTime.Now);

                var dimDate = _dimDateRep.FindOne(x => x.DateKey == dateKey);
                if (dimDate != null)
                {
                    switch (CommonDate)
                    {
